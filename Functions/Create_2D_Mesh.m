...
%                   to generate a 2D mesh in Abaqus                       %
%                                                                         %
...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [status_mesh] = Create_2D_Mesh(dir_name)

global a b Fibre_pos N_fibre S_base Rmax Abaqus;

%% Create Python File for Abaqus CAE with Fibre Distribution

disp('Started creation of Python file');

file_name = strcat(dir_name,'/mesh_plane_strain.py');
...
fprintf(fid,'p = mdb.models[''Model-1''].Part(name=''Fibre_Part_Prov'', dimensionality=TWO_D_PLANAR, \n');
fprintf(fid,'    type=DEFORMABLE_BODY)\n');
fprintf(fid,'p = mdb.models[''Model-1''].parts[''Fibre_Part_Prov''].BaseShell(sketch= \n');
fprintf(fid,'    mdb.models[''Model-1''].sketches[''Fibre_Sketch_Prov''])\n');
...
fprintf(fid,'p = mdb.models[''Model-1''].Part(name=''Fibre_Part_Trim'', dimensionality=TWO_D_PLANAR, \n');
fprintf(fid,'    type=DEFORMABLE_BODY)\n');
fprintf(fid,'p = mdb.models[''Model-1''].parts[''Fibre_Part_Trim''].BaseShell(sketch= \n');
fprintf(fid,'    mdb.models[''Model-1''].sketches[''Fibre_Sketch_Trim''])\n');
fprintf(fid,'s.unsetPrimaryObject()\n');
...
fprintf(fid,'p = mdb.models[''Model-1''].Part(name=''Matrix_Part'', dimensionality=TWO_D_PLANAR, \n');
fprintf(fid,'    type=DEFORMABLE_BODY)\n');
fprintf(fid,'p = mdb.models[''Model-1''].parts[''Matrix_Part''].BaseShell(sketch= \n');
fprintf(fid,'    mdb.models[''Model-1''].sketches[''Matrix_Sketch''])\n');
fprintf(fid,'s.unsetPrimaryObject()\n');
...
dos(strcat(Abaqus,' cae noGUI=',dir_name,'\mesh_plane_strain.py'));
disp(' ');
